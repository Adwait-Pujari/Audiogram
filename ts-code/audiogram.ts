"use strict";
//Creating canvas
let canvas = document.getElementById('grid1') as HTMLCanvasElement | null ;
...
if(ctx != null) {
    ...
    let maxLength = 480;
    let displayValue = 'O';
    let lastClick: [number, number, string] = [0, 0, displayValue];
    ...
    function displayImage() {
        ...
        background.onload = function () {
            let gridRatio = document.getElementById('grid1') as HTMLCanvasElement;
            if(gridRatio instanceof HTMLElement){
    ...
    //Onclick event
    function handleCanvas1Click(event: MouseEvent) {
        if(canvas instanceof HTMLCanvasElement){
    ...
    function confirmDisplayValue(value: string) {
        displayValue = value;
        lastClick = [0, 0, displayValue];
    }
    //To draw the text/letter on the canvas
    function calculateInputText(variable: string, x: number, y:number) {
    ...
    function clearAudiogram() {
        if(canvas instanceof HTMLCanvasElement && ctx instanceof HTMLCanvasElement) {
    ...
    function redrawPoints(drawPoints: DrawPoint[]) {
        if(canvas instanceof HTMLCanvasElement && ctx instanceof HTMLCanvasElement) {
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            // displayImage();
            lastClick = [0, 0, 'O'];
            for (let i = 0; i < drawPoints.length; i++) {
                const data = drawPoints[i];
                const x = data.x;
                const y = data.y;
    ...
    function drawPoint(x: number, y: number, displayValue: string) {
        if( ctx instanceof HTMLCanvasElement) {
